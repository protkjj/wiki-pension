...
def _spot(curve: Dict[int, float], year: int) -> float:
    """만기 year의 spot rate. 커브에 없는 만기는 선형보간, 최대만기 초과는 평평하게 연장."""
    if year in curve:
        return curve[year]
    return interpolate_curve(curve, float(year))


def curve_pv(cashflows: Sequence[Tuple[int, float]], curve: Dict[int, float],
             timing: str = "mid_year") -> float:
    """만기별 spot 커브로 할인한 현금흐름 현재가치."""
    return sum(cf / (1 + _spot(curve, yr)) ** _exponent(yr, timing)
...
def weighted_duration(cashflows: Sequence[Tuple[int, float]], curve: Dict[int, float],
                      timing: str = "mid_year") -> float:
    """가중평균만기(듀레이션) = Σ 시점 × PV(t) / Σ PV(t) (커브 할인 기준)."""
    num = den = 0.0
    for yr, cf in cashflows:
        t = _exponent(yr, timing)
        pv = cf / (1 + _spot(curve, yr)) ** t
        num += t * pv
        den += pv
